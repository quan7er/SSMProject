...
<body class="hold-transition skin-blue sidebar-mini">
	<div class="wrapper">
		...
		<jsp:include page="/WEB-INF/pages/plugins/include_menu_item.jsp">
			...
			<jsp:param name="action" value="news:unlist" />
		</jsp:include>
		<div class="content-wrapper">
			<div class="panel panel-success">
				...
				<div class="panel-body">
					<jsp:include
						page="/WEB-INF/pages/plugins/include_splitpage_search.jsp" />
					<table class="table table-hover">
						...
						<c:forEach items="${allNews}" var="news">
							<tr>
								<td class="text-center"><input type="checkbox" id="nid"
									name="nid" value="1"></td>
								<td class="text-center">${news.title}</td>
								<td class="text-center">${news.mid}</td>
								<td class="text-center">${news.pubdate}</td>
								<td class="text-center">${news.flag == 1?'已发布':'未发布'}</td>
								<td class="text-center"><a href="<%=editNewsUrl%>"
									class="btn btn-xs btn-primary"><span
										class="glyphicon glyphicon-edit"></span>&nbsp;编辑</a></td>
							</tr>
						</c:forEach>
					</table>
					<a href="<%=addNewsUrl%>" id="addBtn"
						class="btn btn-lg btn-primary"><span
						class="glyphicon glyphicon-plus-sign"></span>&nbsp;发布公告</a>
					<button id="removeBtn" class="btn btn-lg btn-danger">
						<span class="glyphicon glyphicon-remove"></span>&nbsp;删除公告
					</button>
					<jsp:include
						page="/WEB-INF/pages/plugins/include_splitpage_bar.jsp" />
				</div>
