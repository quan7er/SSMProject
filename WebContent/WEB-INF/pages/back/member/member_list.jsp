					...
					<table class="table table-hover">
						...
						<c:forEach items="${allMembers}" var="member">
							<tr>
								...
								<td id="regdate-${member.mid}"><fmt:formatDate
										value="${member.regdate}" pattern="" /></td>
								<td id="status-${member.mid}"><c:if
										test="${member.locked == 0 }">
										<span class="text-suceess">活跃</span>
									</c:if> <c:if test="${member.locked == 1 }">
										<span class="text-danger">锁定</span>
									</c:if></td>
								<td><input type="button" id="changeBut-${member.mid}"
									name="changeBut-${member.mid}" value="修改密码"
									class="btn btn-xs btn-default"> <shiro:hasPermission
										name="member:edit">
										<!--<td><input type="button" id="eidt-${member.mid}" name="${member.mid}" value="编辑" class="btn btn-xs btn-primeary"></td>-->
										<a href="<%=editMemberUrl%>?mid=${member.mid}"
											id="edit-${member.mid}" name="edit-${member.mid}"
											class="btn btn-xs btn-primary">编辑</a>
									</shiro:hasPermission> <input type="button"
									id="lockBut-${member.mid}-${member.locked}"
									name="lockBut-${member.mid}-${member.locked}"
									value="${member.locked==1?'激活用户':'锁定用户'}"
									locked="${member.locked}"
									class="btn btn-xs ${member.locked==1?'btn-success':'btn-danger'}">
								</td>
							</tr>
						</c:forEach>
					</table>
								...
								<div class="form-group" id="passwordDiv">
									...
									<div class="col-md-5">
										<input type="text" name="password" id="password"
											class="form-control input-sm" placeholder="请输入雇员真实姓名">
									</div>
