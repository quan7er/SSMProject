...
%>
		<li><a
			href="<%=url%>?cp=<%=currentPage - 1%>&ls=<%=lineSize%>&kw=<%=keyWord%>&col=<%=column%>">上一页</a></li>
		<li class="<%=currentPage == 1 ? "active" : ""%>"><a
			href="<%=url%>?cp=1&ls=<%=lineSize%>&kw=<%=keyWord%>&col=<%=column%>">1</a></li>
		<%
		}
%>
		<%
...
%>
		<li class="<%=currentPage == x ? "active" : ""%>"><a
			href="<%=url%>?cp=<%=x%>&ls=<%=lineSize%>&kw=<%=keyWord%>&col=<%=column%>"><%=x%></a></li>
		<%
			}
...
%>
		<li class="<%=currentPage == pageSize ? "active" : ""%>"><a
			href="<%=url%>?cp=<%=pageSize%>&ls=<%=lineSize%>&kw=<%=keyWord%>&col=<%=column%>"><%=pageSize%></a></li>
		<li><a
			href="<%=url%>?cp=<%=currentPage + 1%>&ls=<%=lineSize%>&kw=<%=keyWord%>&col=<%=column%>">下一页</a></li>
		<%
		}
%>
	</ul>
